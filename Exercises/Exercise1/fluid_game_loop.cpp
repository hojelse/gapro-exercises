...
int main() {
	...
	while (true)
	{
		Update(std::chrono::steady_clock::now() - game_engine.frame_time_end);
		Render();
...
void Update(std::chrono::duration<double> delta_time)
{
	...
	double character_speed = 1 / 0.01; // x unit per y sec
	double dist = character_speed * delta_time.count();
...
void Render()
{
	...
	ITUGames::Console::GotoCoords(floor(game_state.x), floor(game_state.y));
